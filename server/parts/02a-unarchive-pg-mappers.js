...
// Recursively strip null chars from an object/array before it's written to a
// JSONB column — Prisma's napi layer throws when it reads back JSON strings
// containing U+0000 (stored as  in the JSON text representation).
function stripNullBytesDeep(value) {
  ...
  if (value !== null && typeof value === "object") {
    const out = {};
    for (const key of Object.keys(value)) out[key] = stripNullBytesDeep(value[key]);
    return out;
  }
  return value;
}
...
function productToPostgresData(product = {}) {
  ...
  return {
    ...
    images: stripNullBytesDeep(images || {}),
