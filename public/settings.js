...
settingsForm?.addEventListener("submit", async (event) => {
  ...
  try {
    let res = await fetch("/api/settings", { method: "PUT", credentials: "same-origin", headers, body });
    if ([404, 405].includes(res.status)) {
      res = await fetch("/api/settings", { method: "POST", credentials: "same-origin", headers, body });
    }
    if (res.status === 401) {
      window.location.href = "/login.html";
      throw new Error("Требуется вход");
    ...
    statusBox.textContent = error.message || "Не удалось сохранить настройки.";
  }
});
