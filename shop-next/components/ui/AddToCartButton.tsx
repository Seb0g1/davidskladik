...
// Minimalist spray bottle silhouette that fits the perfume brand
function SprayIcon() {
  return (
    <svg
      width="13"
...
export default function AddToCartButton({ product, size = "sm" }: Props) {
  ...
  const handleAdd = useCallback(async (e: React.MouseEvent) => {
    ...
    // Spawn 12 gold particles in a starburst
    setParticles(
    ...
    // Reset fly values and start arc animation
    ...
    await animate(0, 1, {
      ...
      onUpdate: (t) => {
        // Bezier arc: starts center, arcs upper-right, ends fading upper-left
  ...
  return (
    <div style={{ position: "relative", display: "inline-flex", flexShrink: 0 }}>
      ...
      >
        {/* Gold shimmer sweep during launch */}
        {phase === "launching" && (
          <motion.span
            aria-hidden
            initial={{ x: "-130%" }}
        ...
        {/* Invisible ghost for stable sizing */}
        <span
          aria-hidden
          style={{
            opacity: 0,
