...
const guarantees = [
  { icon: "🔬", title: "Прямые поставки", desc: "Мы работаем напрямую с официальными дистрибьюторами и авторизованными импортёрами. Каждая позиция проходит проверку подлинности до попадания на склад." },
  { icon: "📦", title: "Оригинальная упаковка", desc: "Товары хранятся и доставляются в оригинальных фабричных упаковках без вскрытия. Все защитные пломбы и наклейки сохранены." },
  { icon: "🧾", title: "Документы", desc: "По запросу предоставляем подтверждающие документы о происхождении товара (сертификат соответствия, декларация ЕАЭС)." },
  { icon: "💯", title: "100% возврат за подделку", desc: "Если вы докажете, что полученный товар является подделкой, мы вернём полную стоимость заказа без каких-либо условий." },
];

const rights = [
  { article: "Ст. 18 ЗоЗПП", title: "Товар ненадлежащего качества", desc: "Вы вправе потребовать замены, устранения недостатков, соразмерного снижения цены или возврата уплаченной суммы." },
  { article: "Ст. 25 ЗоЗПП", title: "Обмен товара надлежащего качества", desc: "Непродовольственный товар надлежащего качества можно обменять в течение 14 дней, если он не был в употреблении и сохранены потребительские свойства." },
  { article: "Ст. 26.1 ЗоЗПП", title: "Дистанционная продажа", desc: "При покупке через интернет вы вправе отказаться от товара в любое время до его получения или в течение 7 дней после получения без объяснения причин." },
  { article: "Ст. 22 ЗоЗПП", title: "Срок возврата денег", desc: "Требование о возврате денежных средств подлежит удовлетворению в течение 10 дней со дня предъявления соответствующего требования." },
];
      ...
      <div style={{ background: S.bg, minHeight: "100vh" }}>
        <div style={{ maxWidth: 800, margin: "0 auto", padding: "64px clamp(16px,4vw,24px) 96px" }}>
          <nav style={{ fontSize: 12, color: S.muted, marginBottom: 40 }}>
            ...
            {" / "}<span style={{ color: S.text }}>Гарантия</span>
          </nav>
