...
const inputPath = path.join(partsDir, "02a-ozon-unarchive-queue-storage.js");
const lines = fs.readFileSync(inputPath, "utf8").split(/\r?\n/);

const chunks = [
  { name: "02a-ozon-unarchive-queue-helpers.js", start: 1, end: 172 },
  { name: "02a-ozon-unarchive-queue-io.js", start: 174, end: 295 },
  { name: "02a-ozon-unarchive-queue-mutations.js", start: 297, end: lines.length },
];
...
if (!source.includes('"02a-ozon-unarchive-queue-storage.js"')) {
  throw new Error("02a-ozon-unarchive-queue-storage.js not found in source.js");
}
source = source.replace(
  '  "02a-ozon-unarchive-queue-storage.js",\n',
  `${chunks.map((c) => `  "${c.name}",`).join("\n")}\n`,
);
fs.writeFileSync(sourcePath, source);

console.log("split-02a-ozon-unarchive-queue-storage complete", { chunks: chunks.length, totalLines: lines.length });
