...
async function main() {
  ...
  try {
    await sftpPut(conn, path.join(root, "scripts/diag-k18001-pm-rows.cjs"), `${remoteRoot}/scripts/diag-k18001-pm-rows.cjs`);
    await exec(conn, `cd ${remoteRoot} && node scripts/diag-k18001-pm-rows.cjs`, 60000);
  } finally { conn.end(); }
}
