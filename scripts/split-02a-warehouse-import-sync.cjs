...
const inputPath = path.join(partsDir, "02a-warehouse-import-sync.js");
const lines = fs.readFileSync(inputPath, "utf8").split(/\r?\n/);

const chunks = [
  { name: "02a-warehouse-daily-sync.js", start: 1, end: 66 },
  { name: "02a-warehouse-merge-products.js", start: 67, end: 263 },
  { name: "02a-warehouse-import-ozon.js", start: 264, end: 437 },
  { name: "02a-warehouse-import-yandex-sync.js", start: 439, end: lines.length },
];
...
if (!source.includes('"02a-warehouse-import-sync.js"')) {
  throw new Error("02a-warehouse-import-sync.js not found in source.js");
}
source = source.replace(
  '  "02a-warehouse-import-sync.js",\n',
  `${chunks.map((c) => `  "${c.name}",`).join("\n")}\n`,
);
fs.writeFileSync(sourcePath, source);

console.log("split-02a-warehouse-import-sync complete", { chunks: chunks.length, totalLines: lines.length });
