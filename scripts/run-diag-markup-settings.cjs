...
async function main() {
  ...
  try {
    await sftpPut(conn, path.join(__dirname, "diag-markup-settings.cjs"), `${remoteRoot}/scripts/diag-markup-settings.cjs`);
    await exec(conn, `cd ${remoteRoot} && node scripts/diag-markup-settings.cjs`);
  } finally { conn.end(); }
}
