...
const inputPath = path.join(partsDir, "01-bootstrap-auth-storage.js");
const lines = fs.readFileSync(inputPath, "utf8").split(/\r?\n/);

const chunks = [
  { name: "01-bootstrap-auth-core.js", start: 1, end: 172 },
  { name: "01-bootstrap-app-users-storage.js", start: 174, end: lines.length },
];
...
if (!source.includes('"01-bootstrap-auth-storage.js"')) {
  throw new Error("01-bootstrap-auth-storage.js not found in source.js");
}
source = source.replace(
  '  "01-bootstrap-auth-storage.js",\n',
  `${chunks.map((c) => `  "${c.name}",`).join("\n")}\n`,
);
fs.writeFileSync(sourcePath, source);

console.log("split-01-bootstrap-auth-storage complete", { chunks: chunks.length, totalLines: lines.length });
