...
async function main() {
  ...
  try {
    await sftpPut(conn, path.join(root, "scripts/diag-k18001-compute-live.cjs"), `${remoteRoot}/scripts/diag-k18001-compute-live.cjs`);
    await exec(conn, `cd ${remoteRoot} && node scripts/diag-k18001-compute-live.cjs`, 90000);
  } finally { conn.end(); }
}
