...
const inputPath = path.join(partsDir, "02d-supplier-cart-preview.js");
const lines = fs.readFileSync(inputPath, "utf8").split(/\r?\n/);

const chunks = [
  { name: "02d-supplier-cart-resolve.js", start: 1, end: 321 },
  { name: "02d-supplier-cart-build.js", start: 323, end: lines.length },
];
...
if (!source.includes('"02d-supplier-cart-preview.js"')) {
  throw new Error("02d-supplier-cart-preview.js not found in source.js");
}
source = source.replace(
  '  "02d-supplier-cart-preview.js",\n',
  `${chunks.map((c) => `  "${c.name}",`).join("\n")}\n`,
);
fs.writeFileSync(sourcePath, source);

console.log("split-02d-supplier-cart-preview complete", { chunks: chunks.length, totalLines: lines.length });
