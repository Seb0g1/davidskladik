...
async function main() {
  ...
  try {
    await sftpPut(conn, path.join(root, "scripts/diag-k18001-state.cjs"), `${remoteRoot}/scripts/diag-k18001-state.cjs`);
    await exec(conn, `cd ${remoteRoot} && node scripts/diag-k18001-state.cjs`, 60000);
  } finally { conn.end(); }
}
