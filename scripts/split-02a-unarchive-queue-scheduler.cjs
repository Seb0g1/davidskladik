...
const inputPath = path.join(partsDir, "02a-unarchive-queue-scheduler.js");
const lines = fs.readFileSync(inputPath, "utf8").split(/\r?\n/);

const chunks = [
  { name: "02a-price-retry-queue-items.js", start: 1, end: 182 },
  { name: "02a-price-history-storage.js", start: 183, end: 352 },
  { name: "02a-price-retry-scheduler.js", start: 354, end: lines.length },
];
...
if (!source.includes('"02a-unarchive-queue-scheduler.js"')) {
  throw new Error("02a-unarchive-queue-scheduler.js not found in source.js");
}
source = source.replace(
  '  "02a-unarchive-queue-scheduler.js",\n',
  `${chunks.map((c) => `  "${c.name}",`).join("\n")}\n`,
);
fs.writeFileSync(sourcePath, source);

console.log("split-02a-unarchive-queue-scheduler complete", { chunks: chunks.length, totalLines: lines.length });
