...
const inputPath = path.join(partsDir, "02d-routes-prices-finance.js");
const lines = fs.readFileSync(inputPath, "utf8").split(/\r?\n/);

const chunks = [
  { name: "02d-prices-finance-routes-sales.js", start: 1, end: 325 },
  { name: "02d-prices-finance-core.js", start: 326, end: 967 },
  { name: "02d-prices-finance-routes-api.js", start: 968, end: lines.length },
];
...
if (!source.includes('"02d-routes-prices-finance.js"')) {
  throw new Error("02d-routes-prices-finance.js not found in source.js");
}
source = source.replace(
  '  "02d-routes-prices-finance.js",\n',
  `${chunks.map((c) => `  "${c.name}",`).join("\n")}\n`,
);
fs.writeFileSync(sourcePath, source);

console.log("split-02d-routes-prices-finance complete", { chunks: chunks.length, totalLines: lines.length });
