...
const inputPath = path.join(partsDir, "02d-routes-warehouse-ai.js");
const lines = fs.readFileSync(inputPath, "utf8").split(/\r?\n/);

const chunks = [
  { name: "02d-warehouse-ai-routes-early.js", start: 1, end: 638 },
  { name: "02d-warehouse-ai-helpers.js", start: 639, end: 729 },
  { name: "02d-warehouse-ai-routes-late.js", start: 731, end: lines.length },
];
...
if (!source.includes('"02d-routes-warehouse-ai.js"')) {
  throw new Error("02d-routes-warehouse-ai.js not found in source.js");
}
source = source.replace(
  '  "02d-routes-warehouse-ai.js",\n',
  `${chunks.map((c) => `  "${c.name}",`).join("\n")}\n`,
);
fs.writeFileSync(sourcePath, source);

console.log("split-02d-routes-warehouse-ai complete", { chunks: chunks.length, totalLines: lines.length });
