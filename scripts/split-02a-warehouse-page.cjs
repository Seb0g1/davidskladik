...
const inputPath = path.join(partsDir, "02a-warehouse-page.js");
const lines = fs.readFileSync(inputPath, "utf8").split(/\r?\n/);

const chunks = [
  { name: "02a-warehouse-page-counts.js", start: 1, end: 397 },
  { name: "02a-warehouse-page-build.js", start: 398, end: 799 },
  { name: "02a-warehouse-page-display.js", start: 800, end: lines.length },
];
...
if (!source.includes('"02a-warehouse-page.js"')) {
  throw new Error("02a-warehouse-page.js not found in source.js");
}
source = source.replace(
  '  "02a-warehouse-page.js",\n',
  `${chunks.map((c) => `  "${c.name}",`).join("\n")}\n`,
);
fs.writeFileSync(sourcePath, source);

console.log("split-02a-warehouse-page complete", { chunks: chunks.length, totalLines: lines.length });
