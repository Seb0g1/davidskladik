...
const inputPath = path.join(partsDir, "02a-unarchive-postgres-sync.js");
const lines = fs.readFileSync(inputPath, "utf8").split(/\r?\n/);

const chunks = [
  { name: "02a-unarchive-pg-mappers.js", start: 1, end: 89 },
  { name: "02a-unarchive-pg-upsert.js", start: 90, end: 189 },
  { name: "02a-unarchive-pg-links-cache.js", start: 190, end: 270 },
  { name: "02a-unarchive-pg-brands-backfill.js", start: 271, end: lines.length },
];
...
if (!source.includes('"02a-unarchive-postgres-sync.js"')) {
  throw new Error("02a-unarchive-postgres-sync.js not found in source.js");
}
source = source.replace(
  '  "02a-unarchive-postgres-sync.js",\n',
  `${chunks.map((c) => `  "${c.name}",`).join("\n")}\n`,
);
fs.writeFileSync(sourcePath, source);

console.log("split-02a-unarchive-postgres-sync complete", { chunks: chunks.length, totalLines: lines.length });
