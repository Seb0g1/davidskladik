...
async function main() {
  const arg = process.argv[2] || "234123";
  const conn = await connect();
  try {
    await sftpPut(conn, path.join(__dirname, "diag-product-markup.cjs"), `${remoteRoot}/scripts/diag-product-markup.cjs`);
    await exec(conn, `cd ${remoteRoot} && node scripts/diag-product-markup.cjs ${arg}`);
  } finally { conn.end(); }
}
