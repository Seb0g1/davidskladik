...
async function main() {
  ...
  try {
    ...
    console.log("=== verify delete <20ml (dry-run) ===");
    await exec(conn, `cd ${remoteRoot} && node scripts/delete-yandex-small-volume.cjs --dry-run`);
    console.log("=== verify repair (dry-run) ===");
    await exec(conn, `cd ${remoteRoot} && node scripts/repair-yandex-media-from-ozon.cjs --dry-run`);
