...
async function main() {
  ...
  try {
    await sftpPut(conn, path.join(root, "scripts/diag-k18001-clamp.cjs"), `${remoteRoot}/scripts/diag-k18001-clamp.cjs`);
    await exec(conn, `cd ${remoteRoot} && node scripts/diag-k18001-clamp.cjs`, 60000);
  } finally { conn.end(); }
}
