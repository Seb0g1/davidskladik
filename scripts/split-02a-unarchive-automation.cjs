...
const inputPath = path.join(partsDir, "02a-unarchive-automation.js");
const lines = fs.readFileSync(inputPath, "utf8").split(/\r?\n/);

const chunks = [
  { name: "02a-unarchive-queue-scheduler.js", start: 1, end: 403 },
  { name: "02a-unarchive-postgres-sync.js", start: 405, end: 797 },
  { name: "02a-unarchive-backfill.js", start: 798, end: lines.length },
];
...
if (!source.includes('"02a-unarchive-automation.js"')) {
  throw new Error("02a-unarchive-automation.js not found in source.js");
}
source = source.replace(
  '  "02a-unarchive-automation.js",\n',
  `${chunks.map((c) => `  "${c.name}",`).join("\n")}\n`,
);
fs.writeFileSync(sourcePath, source);

console.log("split-02a-unarchive-automation complete", { chunks: chunks.length, totalLines: lines.length });
