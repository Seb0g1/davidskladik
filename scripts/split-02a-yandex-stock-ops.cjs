...
const inputPath = path.join(partsDir, "02a-yandex-stock-ops.js");
const lines = fs.readFileSync(inputPath, "utf8").split(/\r?\n/);

const chunks = [
  { name: "02a-yandex-cleanup-ops.js", start: 1, end: 97 },
  { name: "02a-yandex-stock-payload.js", start: 98, end: 206 },
  { name: "02a-yandex-stock-send-chunk.js", start: 207, end: 315 },
  { name: "02a-yandex-stock-send-bulk.js", start: 317, end: lines.length },
];
...
if (!source.includes('"02a-yandex-stock-ops.js"')) {
  throw new Error("02a-yandex-stock-ops.js not found in source.js");
}
source = source.replace(
  '  "02a-yandex-stock-ops.js",\n',
  `${chunks.map((c) => `  "${c.name}",`).join("\n")}\n`,
);
fs.writeFileSync(sourcePath, source);

console.log("split-02a-yandex-stock-ops complete", { chunks: chunks.length, totalLines: lines.length });
