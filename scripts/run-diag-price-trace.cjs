...
async function main() {
  ...
  try {
    await sftpPut(conn, path.join(root, "scripts/diag-k18001-price-trace.cjs"), `${remoteRoot}/scripts/diag-k18001-price-trace.cjs`);
    await exec(conn, `cd ${remoteRoot} && node scripts/diag-k18001-price-trace.cjs`, 120000);
  } finally { conn.end(); }
}
