...
const inputPath = path.join(partsDir, "02a-yandex-ozon-catalog.js");
const lines = fs.readFileSync(inputPath, "utf8").split(/\r?\n/);

const chunks = [
  { name: "02a-yandex-ozon-ozon-api.js", start: 1, end: 389 },
  { name: "02a-yandex-ozon-yandex-read.js", start: 390, end: 519 },
  { name: "02a-yandex-ozon-price-send.js", start: 520, end: 880 },
  { name: "02a-yandex-ozon-warehouse-normalize.js", start: 881, end: lines.length },
];
...
if (!source.includes('"02a-yandex-ozon-catalog.js"')) {
  throw new Error("02a-yandex-ozon-catalog.js not found in source.js");
}
source = source.replace(
  '  "02a-yandex-ozon-catalog.js",\n',
  `${chunks.map((c) => `  "${c.name}",`).join("\n")}\n`,
);
fs.writeFileSync(sourcePath, source);

console.log("split-02a-yandex-ozon-catalog complete", { chunks: chunks.length, totalLines: lines.length });
