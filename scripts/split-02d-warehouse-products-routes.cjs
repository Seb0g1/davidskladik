...
const inputPath = path.join(partsDir, "02d-routes-warehouse-products.js");
const lines = fs.readFileSync(inputPath, "utf8").split(/\r?\n/);

const chunks = [
  { name: "02d-warehouse-products-crud.js", start: 1, end: 57 },
  { name: "02d-warehouse-ai-image-jobs.js", start: 59, end: 301 },
  { name: "02d-warehouse-premium-image-assets.js", start: 302, end: 496 },
  { name: "02d-warehouse-ai-image-runners.js", start: 497, end: lines.length },
];
...
if (!source.includes('"02d-routes-warehouse-products.js"')) {
  throw new Error("02d-routes-warehouse-products.js not found in source.js");
}
source = source.replace(
  '  "02d-routes-warehouse-products.js",\n',
  `${chunks.map((c) => `  "${c.name}",`).join("\n")}\n`,
);
fs.writeFileSync(sourcePath, source);

console.log("split-02d-warehouse-products-routes complete", { chunks: chunks.length, totalLines: lines.length });
