...
const inputPath = path.join(partsDir, "02d-routes-suppliers.js");
const lines = fs.readFileSync(inputPath, "utf8").split(/\r?\n/);

const chunks = [
  { name: "02d-suppliers-routes-read.js", start: 1, end: 217 },
  { name: "02d-suppliers-routes-search.js", start: 218, end: 367 },
  { name: "02d-suppliers-routes-write.js", start: 368, end: lines.length },
];
...
if (!source.includes('"02d-routes-suppliers.js"')) {
  throw new Error("02d-routes-suppliers.js not found in source.js");
}
source = source.replace(
  '  "02d-routes-suppliers.js",\n',
  `${chunks.map((c) => `  "${c.name}",`).join("\n")}\n`,
);
fs.writeFileSync(sourcePath, source);

console.log("split-02d-routes-suppliers complete", { chunks: chunks.length, totalLines: lines.length });
