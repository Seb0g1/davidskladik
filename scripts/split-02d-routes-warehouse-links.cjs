...
const inputPath = path.join(partsDir, "02d-routes-warehouse-links.js");
const lines = fs.readFileSync(inputPath, "utf8").split(/\r?\n/);

const chunks = [
  { name: "02d-warehouse-links-routes-bulk.js", start: 1, end: 313 },
  { name: "02d-warehouse-links-mutation.js", start: 314, end: 522 },
  { name: "02d-warehouse-links-routes-manage.js", start: 523, end: lines.length },
];
...
if (!source.includes('"02d-routes-warehouse-links.js"')) {
  throw new Error("02d-routes-warehouse-links.js not found in source.js");
}
source = source.replace(
  '  "02d-routes-warehouse-links.js",\n',
  `${chunks.map((c) => `  "${c.name}",`).join("\n")}\n`,
);
fs.writeFileSync(sourcePath, source);

console.log("split-02d-routes-warehouse-links complete", { chunks: chunks.length, totalLines: lines.length });
