...
async function main() {
  ...
  try {
    const localScript = path.join(__dirname, "_tmp_unarchive_diag_remote.cjs");
    const remoteScript = `${remoteRoot}/scripts/_tmp_unarchive_diag_remote.cjs`;
    console.log("Uploading...");
    await sftpPut(conn, localScript, remoteScript);
    console.log("Running...\n");
    await exec(conn, `cd ${remoteRoot} && node scripts/_tmp_unarchive_diag_remote.cjs`, 120000);
  } finally {
    conn.end();
  }
}
