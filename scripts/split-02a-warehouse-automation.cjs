...
const inputPath = path.join(partsDir, "02a-warehouse-automation.js");
const lines = fs.readFileSync(inputPath, "utf8").split(/\r?\n/);

const chunks = [
  { name: "02a-warehouse-product-diagnostics.js", start: 1, end: 179 },
  { name: "02a-warehouse-sku-diagnostics.js", start: 181, end: 275 },
  { name: "02a-warehouse-group-detail.js", start: 277, end: 390 },
  { name: "02a-warehouse-fast-page.js", start: 392, end: 553 },
  { name: "02a-warehouse-history-offers.js", start: 555, end: lines.length },
];
...
if (!source.includes('"02a-warehouse-automation.js"')) {
  throw new Error("02a-warehouse-automation.js not found in source.js");
}
source = source.replace(
  '  "02a-warehouse-automation.js",\n',
  `${chunks.map((c) => `  "${c.name}",`).join("\n")}\n`,
);
fs.writeFileSync(sourcePath, source);

console.log("split-02a-warehouse-automation complete", { chunks: chunks.length, totalLines: lines.length });
