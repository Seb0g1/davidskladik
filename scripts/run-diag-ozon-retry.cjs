...
async function main() {
  ...
  try {
    await sftpPut(conn, path.join(__dirname, "diag-ozon-retry-detail.cjs"), `${remoteRoot}/scripts/diag-ozon-retry-detail.cjs`);
    await exec(conn, `cd ${remoteRoot} && node scripts/diag-ozon-retry-detail.cjs`);
  } finally { conn.end(); }
}
