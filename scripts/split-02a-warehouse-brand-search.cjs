...
const inputPath = path.join(partsDir, "02a-warehouse-brand-search.js");
const lines = fs.readFileSync(inputPath, "utf8").split(/\r?\n/);

const chunks = [
  { name: "02a-brand-index-core.js", start: 1, end: 281 },
  { name: "02a-brand-index-postgres.js", start: 283, end: 370 },
  { name: "02a-price-master-match-helpers.js", start: 372, end: lines.length },
];
...
if (!source.includes('"02a-warehouse-brand-search.js"')) {
  throw new Error("02a-warehouse-brand-search.js not found in source.js");
}
source = source.replace(
  '  "02a-warehouse-brand-search.js",\n',
  `${chunks.map((c) => `  "${c.name}",`).join("\n")}\n`,
);
fs.writeFileSync(sourcePath, source);

console.log("split-02a-warehouse-brand-search complete", { chunks: chunks.length, totalLines: lines.length });
