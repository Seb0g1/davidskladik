...
async function main() {
  ...
  try {
    await sftpPut(conn, path.join(root, "scripts/diag-warehouse-json-supplier.cjs"), `${remoteRoot}/scripts/diag-warehouse-json-supplier.cjs`);
    await exec(conn, `cd ${remoteRoot} && node scripts/diag-warehouse-json-supplier.cjs`, 120000);
  } finally { conn.end(); }
}
