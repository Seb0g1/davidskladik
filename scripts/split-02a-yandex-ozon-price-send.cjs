...
const inputPath = path.join(partsDir, "02a-yandex-ozon-price-send.js");
const lines = fs.readFileSync(inputPath, "utf8").split(/\r?\n/);

const chunks = [
  { name: "02a-yandex-price-helpers.js", start: 1, end: 151 },
  { name: "02a-yandex-price-build-send.js", start: 152, end: lines.length },
];
...
if (!source.includes('"02a-yandex-ozon-price-send.js"')) {
  throw new Error("02a-yandex-ozon-price-send.js not found in source.js");
}
source = source.replace(
  '  "02a-yandex-ozon-price-send.js",\n',
  `${chunks.map((c) => `  "${c.name}",`).join("\n")}\n`,
);
fs.writeFileSync(sourcePath, source);

console.log("split-02a-yandex-ozon-price-send complete", { chunks: chunks.length, totalLines: lines.length });
