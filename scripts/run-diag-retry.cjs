...
async function main() {
  ...
  try {
    await sftpPut(conn, path.join(root, "scripts/diag-k18001-retry.cjs"), `${remoteRoot}/scripts/diag-k18001-retry.cjs`);
    await exec(conn, `cd ${remoteRoot} && node scripts/diag-k18001-retry.cjs`, 60000);
  } finally { conn.end(); }
}
