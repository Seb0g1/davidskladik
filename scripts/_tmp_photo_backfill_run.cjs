...
const remoteScript = remoteRoot + "/scripts/_tmp_photo_backfill_run.js";
...
const runScript = [
  ...
  '  console.log("Running photo backfill (limit 5000, wait)...");',
  '  const res = await request("POST", "/api/yandex/photo-backfill", { cookie, body: { limit: 5000, wait: true } });',
  '  console.log("HTTP " + res.status);',
  '  console.log(JSON.stringify(res.body, null, 2));',
  '  if (res.status >= 400) process.exit(1);',
  '}',
  'main().catch((e) => { console.error("BACKFILL_FAILED:", e.message); process.exit(1); });',
].join("\n");
