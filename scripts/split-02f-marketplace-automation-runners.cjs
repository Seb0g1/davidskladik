...
const inputPath = path.join(partsDir, "02f-marketplace-automation-runners.js");
const lines = fs.readFileSync(inputPath, "utf8").split(/\r?\n/);

const chunks = [
  { name: "02f-no-supplier-marketplace-automation.js", start: 1, end: 113 },
  { name: "02f-supplier-recovery-automation.js", start: 114, end: lines.length },
];
...
if (!source.includes('"02f-marketplace-automation-runners.js"')) {
  throw new Error("02f-marketplace-automation-runners.js not found in source.js");
}
source = source.replace(
  '  "02f-marketplace-automation-runners.js",\n',
  `${chunks.map((c) => `  "${c.name}",`).join("\n")}\n`,
);
fs.writeFileSync(sourcePath, source);

console.log("split-02f-marketplace-automation-runners complete", { chunks: chunks.length, totalLines: lines.length });
