...
async function main() {
  ...
  try {
    await sftpPut(conn, path.join(__dirname, "diag-supplier-cart.cjs"), `${remoteRoot}/scripts/diag-supplier-cart.cjs`);
    await exec(conn, `cd ${remoteRoot} && node scripts/diag-supplier-cart.cjs`);
  } finally { conn.end(); }
}
