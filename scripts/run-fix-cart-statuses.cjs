...
async function main() {
  ...
  try {
    await sftpPut(conn, path.join(__dirname, "fix-cart-statuses-remote.cjs"), `${remoteRoot}/scripts/fix-cart-statuses-remote.cjs`);
    await exec(conn, `cd ${remoteRoot} && node scripts/fix-cart-statuses-remote.cjs`);
  } finally { conn.end(); }
}
