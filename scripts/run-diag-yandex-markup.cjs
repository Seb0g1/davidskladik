...
async function main() {
  ...
  try {
    await sftpPut(conn, path.join(__dirname, "diag-yandex-markup.cjs"), `${remoteRoot}/scripts/diag-yandex-markup.cjs`);
    await exec(conn, `cd ${remoteRoot} && node scripts/diag-yandex-markup.cjs`);
  } finally { conn.end(); }
}
