...
const inputPath = path.join(partsDir, "02f-supplier-cart-routes.js");
const lines = fs.readFileSync(inputPath, "utf8").split(/\r?\n/);

const chunks = [
  { name: "02f-supplier-cart-audit-public.js", start: 1, end: 39 },
  { name: "02f-supplier-cart-routes.js", start: 41, end: 183 },
  { name: "02f-supplier-picking-routes.js", start: 185, end: lines.length },
];
...
const keepsInputName = chunks.some((chunk) => chunk.name === inputBasename);
if (!keepsInputName) {
  fs.unlinkSync(inputPath);
}

let source = fs.readFileSync(sourcePath, "utf8");
if (!source.includes('"02f-supplier-cart-routes.js"')) {
  throw new Error("02f-supplier-cart-routes.js not found in source.js");
}
...
console.log("split-02f-supplier-cart-routes complete", { chunks: chunks.length, totalLines: lines.length });
