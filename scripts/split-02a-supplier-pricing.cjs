...
const inputPath = path.join(partsDir, "02a-supplier-pricing.js");
const lines = fs.readFileSync(inputPath, "utf8").split(/\r?\n/);

const chunks = [
  { name: "02a-supplier-pricing-normalize.js", start: 1, end: 312 },
  { name: "02a-supplier-pricing-audit-rate.js", start: 314, end: lines.length },
];
...
if (!source.includes('"02a-supplier-pricing.js"')) {
  throw new Error("02a-supplier-pricing.js not found in source.js");
}
source = source.replace(
  '  "02a-supplier-pricing.js",\n',
  `${chunks.map((c) => `  "${c.name}",`).join("\n")}\n`,
);
fs.writeFileSync(sourcePath, source);

console.log("split-02a-supplier-pricing complete", { chunks: chunks.length, totalLines: lines.length });
