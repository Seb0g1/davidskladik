...
const inputPath = path.join(partsDir, "02a-price-master-warehouse-view.js");
const lines = fs.readFileSync(inputPath, "utf8").split(/\r?\n/);

const chunks = [
  { name: "02a-price-master-warehouse-helpers.js", start: 1, end: 192 },
  { name: "02a-price-master-warehouse-maps.js", start: 194, end: 295 },
  { name: "02a-price-master-warehouse-build.js", start: 297, end: lines.length },
];
...
if (!source.includes('"02a-price-master-warehouse-view.js"')) {
  throw new Error("02a-price-master-warehouse-view.js not found in source.js");
}
source = source.replace(
  '  "02a-price-master-warehouse-view.js",\n',
  `${chunks.map((c) => `  "${c.name}",`).join("\n")}\n`,
);
fs.writeFileSync(sourcePath, source);

console.log("split-02a-price-master-warehouse-view complete", { chunks: chunks.length, totalLines: lines.length });
