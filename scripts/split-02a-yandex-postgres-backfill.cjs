...
const inputPath = path.join(partsDir, "02a-yandex-postgres-backfill.js");
const lines = fs.readFileSync(inputPath, "utf8").split(/\r?\n/);

const chunks = [
  { name: "02a-yandex-pg-sibling-fetch.js", start: 1, end: 97 },
  { name: "02a-yandex-pg-backfill-manual-groups.js", start: 99, end: 243 },
  { name: "02a-yandex-pg-normalize-targets.js", start: 245, end: 296 },
  { name: "02a-yandex-pg-backfill-auto-groups.js", start: 298, end: lines.length },
];
...
if (!source.includes('"02a-yandex-postgres-backfill.js"')) {
  throw new Error("02a-yandex-postgres-backfill.js not found in source.js");
}
source = source.replace(
  '  "02a-yandex-postgres-backfill.js",\n',
  `${chunks.map((c) => `  "${c.name}",`).join("\n")}\n`,
);
fs.writeFileSync(sourcePath, source);

console.log("split-02a-yandex-postgres-backfill complete", { chunks: chunks.length, totalLines: lines.length });
