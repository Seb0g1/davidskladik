...
async function main() {
  ...
  try {
    await sftpPut(conn, path.join(__dirname, "diag-prices-db.cjs"), `${remoteRoot}/scripts/diag-prices-db.cjs`);
    await exec(conn, `cd ${remoteRoot} && node scripts/diag-prices-db.cjs`);
  } finally {
    conn.end();
  }
}
