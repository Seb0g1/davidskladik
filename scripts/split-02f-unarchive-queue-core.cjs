...
const inputPath = path.join(partsDir, "02f-unarchive-queue-core.js");
const lines = fs.readFileSync(inputPath, "utf8").split(/\r?\n/);

const chunks = [
  { name: "02f-unarchive-queue-rebuild.js", start: 1, end: 97 },
  { name: "02f-unarchive-products-core.js", start: 98, end: 277 },
  { name: "02f-unarchive-verify-actions.js", start: 278, end: lines.length },
];
...
if (!source.includes('"02f-unarchive-queue-core.js"')) {
  throw new Error("02f-unarchive-queue-core.js not found in source.js");
}
source = source.replace(
  '  "02f-unarchive-queue-core.js",\n',
  `${chunks.map((c) => `  "${c.name}",`).join("\n")}\n`,
);
fs.writeFileSync(sourcePath, source);

console.log("split-02f-unarchive-queue-core complete", { chunks: chunks.length, totalLines: lines.length });
