...
async function main() {
  ...
  try {
    await sftpPut(conn, path.join(root, "scripts/fix-k18001-prices.cjs"), `${remoteRoot}/scripts/fix-k18001-prices.cjs`);
    const args = DRY_RUN ? " --dry-run" : "";
    await exec(conn, `cd ${remoteRoot} && node scripts/fix-k18001-prices.cjs${args}`, 120000);
  } finally { conn.end(); }
}
