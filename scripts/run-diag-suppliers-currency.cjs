...
async function main() {
  ...
  try {
    await sftpPut(conn, path.join(root, "scripts/diag-suppliers-currency.cjs"), `${remoteRoot}/scripts/diag-suppliers-currency.cjs`);
    await exec(conn, `cd ${remoteRoot} && node scripts/diag-suppliers-currency.cjs`, 60000);
  } finally { conn.end(); }
}
