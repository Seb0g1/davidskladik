...
const inputPath = path.join(partsDir, "02a-utils-marketplace-images.js");
const lines = fs.readFileSync(inputPath, "utf8").split(/\r?\n/);

const chunks = [
  { name: "02a-utils-image-url-validate.js", start: 1, end: 159 },
  { name: "02a-utils-image-file-helpers.js", start: 161, end: 302 },
  { name: "02a-utils-ozon-price-payload.js", start: 304, end: lines.length },
];
...
if (!source.includes('"02a-utils-marketplace-images.js"')) {
  throw new Error("02a-utils-marketplace-images.js not found in source.js");
}
source = source.replace(
  '  "02a-utils-marketplace-images.js",\n',
  `${chunks.map((c) => `  "${c.name}",`).join("\n")}\n`,
);
fs.writeFileSync(sourcePath, source);

console.log("split-02a-utils-marketplace-images complete", { chunks: chunks.length, totalLines: lines.length });
