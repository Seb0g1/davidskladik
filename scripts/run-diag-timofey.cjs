...
async function main() {
  ...
  try {
    await sftpPut(conn, path.join(root, "scripts/diag-timofey-currency.cjs"), `${remoteRoot}/scripts/diag-timofey-currency.cjs`);
    await exec(conn, `cd ${remoteRoot} && node scripts/diag-timofey-currency.cjs`, 60000);
  } finally { conn.end(); }
}
