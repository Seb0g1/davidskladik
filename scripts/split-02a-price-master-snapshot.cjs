...
const inputPath = path.join(partsDir, "02a-price-master-snapshot.js");
const lines = fs.readFileSync(inputPath, "utf8").split(/\r?\n/);

const chunks = [
  { name: "02a-snapshot-core.js", start: 1, end: 295 },
  { name: "02a-ozon-unarchive-queue-storage.js", start: 296, end: 648 },
  { name: "02a-yandex-unarchive-queue-storage.js", start: 649, end: lines.length },
];
...
if (!source.includes('"02a-price-master-snapshot.js"')) {
  throw new Error("02a-price-master-snapshot.js not found in source.js");
}
source = source.replace(
  '  "02a-price-master-snapshot.js",\n',
  `${chunks.map((c) => `  "${c.name}",`).join("\n")}\n`,
);
fs.writeFileSync(sourcePath, source);

console.log("split-02a-price-master-snapshot complete", { chunks: chunks.length, totalLines: lines.length });
