...
const inputPath = path.join(partsDir, "02a-normalizers-links.js");
const lines = fs.readFileSync(inputPath, "utf8").split(/\r?\n/);

const chunks = [
  { name: "02a-normalizers-drafts.js", start: 1, end: 281 },
  { name: "02a-normalizers-products.js", start: 282, end: 402 },
  { name: "02a-normalizers-warehouse-links.js", start: 403, end: 688 },
  { name: "02a-normalizers-price-breakdown.js", start: 689, end: lines.length },
];
...
if (!source.includes('"02a-normalizers-links.js"')) {
  throw new Error("02a-normalizers-links.js not found in source.js");
}
source = source.replace(
  '  "02a-normalizers-links.js",\n',
  `${chunks.map((c) => `  "${c.name}",`).join("\n")}\n`,
);
fs.writeFileSync(sourcePath, source);

console.log("split-02a-normalizers-links complete", { chunks: chunks.length, totalLines: lines.length });
