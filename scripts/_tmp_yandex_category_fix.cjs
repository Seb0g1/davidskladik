#!/usr/bin/env node
"use strict";
// Отчёт об ошибках карточек Яндекса + исправление чужих категорий: дергает на
// проде POST /api/ozon-yandex-import/fix-yandex-categories (dry-run по
// умолчанию, --confirmed = реальная отправка категорий в Яндекс).
const { Client } = require("ssh2");
const password = process.env.DEPLOY_PASSWORD;
if (!password) { console.error("DEPLOY_PASSWORD required"); process.exit(1); }
const confirmed = process.argv.includes("--confirmed");
const remoteRoot = "/var/www/davidsklad/davidskladik";
const remoteScript = remoteRoot + "/scripts/_tmp_yandex_category_fix.js";
...
const runScript = [
  ...
  'function request(method, urlPath, { cookie = "", body = null, timeoutMs = 30 * 60_000 } = {}) {',
  ...
  '  console.log("fix-yandex-categories " + (confirmed ? "(CONFIRMED: отправка категорий в Яндекс)" : "(dry run)") + "...");',
  '  const res = await request("POST", "/api/ozon-yandex-import/fix-yandex-categories", { cookie, body });',
  '  console.log("HTTP " + res.status);',
  '  console.log(JSON.stringify(res.body, null, 2));',
  '  if (res.status >= 400) process.exit(1);',
  '}',
  'main().catch((e) => { console.error("CATEGORY_FIX_FAILED:", e.message); process.exit(1); });',
].join("\n");
