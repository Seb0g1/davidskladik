...
async function main() {
  ...
  try {
    await sftpPut(conn, path.join(root, "scripts/fix-k18001-stale-clear.cjs"), `${remoteRoot}/scripts/fix-k18001-stale-clear.cjs`);
    await exec(conn, `cd ${remoteRoot} && node scripts/fix-k18001-stale-clear.cjs`, 60000);
  } finally { conn.end(); }
}
