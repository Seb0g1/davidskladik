...
async function main() {
  ...
  try {
    await sftpPut(conn, path.join(__dirname, "diag-prices-all.cjs"), `${remoteRoot}/scripts/diag-prices-all.cjs`);
    await exec(conn, `cd ${remoteRoot} && node scripts/diag-prices-all.cjs`);
  } finally { conn.end(); }
}
